#include "message.h"
#include <QDebug>

const char CRC8Table[]={
    0, 94, 188, 226, 97, 63, 221, 131, 194, 156, 126, 32, 163, 253, 31, 65,
    157, 195, 33, 127, 252, 162, 64, 30, 95, 1, 227, 189, 62, 96, 130, 220,
    35, 125, 159, 193, 66, 28, 254, 160, 225, 191, 93, 3, 128, 222, 60, 98,
    190, 224, 2, 92, 223, 129, 99, 61, 124, 34, 192, 158, 29, 67, 161, 255,
    70, 24, 250, 164, 39, 121, 155, 197, 132, 218, 56, 102, 229, 187, 89, 7,
    219, 133, 103, 57, 186, 228, 6, 88, 25, 71, 165, 251, 120, 38, 196, 154,
    101, 59, 217, 135, 4, 90, 184, 230, 167, 249, 27, 69, 198, 152, 122, 36,
    248, 166, 68, 26, 153, 199, 37, 123, 58, 100, 134, 216, 91, 5, 231, 185,
    140, 210, 48, 110, 237, 179, 81, 15, 78, 16, 242, 172, 47, 113, 147, 205,
    17, 79, 173, 243, 112, 46, 204, 146, 211, 141, 111, 49, 178, 236, 14, 80,
    175, 241, 19, 77, 206, 144, 114, 44, 109, 51, 209, 143, 12, 82, 176, 238,
    50, 108, 142, 208, 83, 13, 239, 177, 240, 174, 76, 18, 145, 207, 45, 115,
    202, 148, 118, 40, 171, 245, 23, 73, 8, 86, 180, 234, 105, 55, 213, 139,
    87, 9, 235, 181, 54, 104, 138, 212, 149, 203, 41, 119, 244, 170, 72, 22,
    233, 183, 85, 11, 136, 214, 52, 106, 43, 117, 151, 201, 74, 20, 246, 168,
    116, 42, 200, 150, 21, 75, 169, 247, 182, 232, 10, 84, 215, 137, 107, 53
};
const char SEND_FLAG = 0xAA;
const char RECV_FLAG = 0xBB;
const char ENTER_TEST_MODE = 0x81;
const char GET_NORDIC_SOFTVER = 0x02;
const char GET_NORDIC_SN = 0x83;

message::message()
{
    for(int i = 0; i < sizeof(CRC8Table); i++)
    {
        CRC8TableArray.append((unsigned char)CRC8Table[i]);
    }
}

char message::CRC8(const char *p, int counter)
{
    char crc8 = 0;
    for( ; counter > 0; counter--){
        crc8 = CRC8Table[(crc8^*p) & 0xff];
//         qDebug("crc8=%x,*p=%x", (unsigned char)crc8, (unsigned char)*p);
        p++;

    }
    return(crc8);
}

QByteArray message::GetMessageByType(const char* type, int len)
{
    QByteArray ba;
    ba.resize(32);
    ba[0] = SEND_FLAG;
    for(int i = 0; i < len; i++)
    {
        ba[i+1] = type[i];
    }
    for(int i = 0; i < 30 - len; i++)
    {
        ba[len+i+1] = 0;
    }
    char *buf = ba.data();
    int len = ba.size();
    ba[31] = CRC8(buf, len - 1);
//    ba[31] = 0;
    for(int i = 0; i < ba.size(); i++)
    {
        qDebug("ba[%d]=%x,", i, (unsigned char)ba[i]);
    }
//    for(int i = 0; i < sizeof(CRC8Table); i++)
//    {
//        qDebug("crc8table[%x]=%x,", i, (unsigned char)CRC8Table[i]);
//    }
    return ba;
}

QByteArray message::PackEnterTestMode()
{
    return GetMessageByType(&ENTER_TEST_MODE, 1);
//    qDebug() << "EnterTestMode";
//    for(int i = 0; i < ba.size(); i++)
//    {
//        qDebug("%x,", (unsigned char)ba[i]);
//    }
}

QByteArray message::PackGetNordicSoftVer()
{
    return GetMessageByType(&GET_NORDIC_SOFTVER, 1);
}

QByteArray message::PackGetNordicSN()
{
    return GetMessageByType(&GET_NORDIC_SN, 1);
}

bool message::IsReplyPacket(QByteArray ba)
{
    if(ba.size() != 32)
    {
        qDebug() << "recv size is not 32";
        qDebug() << "recv size = " << ba.size();
        for(int i = 0; i < ba.size(); i++)
        {
            qDebug("ba[%d]=%x,", i, (unsigned char)ba[i]);
        }
        return false;
    }
//    for(int i = 0; i < ba.size(); i++)
//    {
//        qDebug("ba[%d]=%x,", i, (unsigned char)ba[i]);
//    }
    char *buf = ba.data();
    int len = ba.size();
//    qDebug("(unsigned char)buf[0]=%x", (unsigned char)buf[0]);
    if((unsigned char)buf[0] != (RECV_FLAG&0xff))
    {
        qDebug() << "recv data not begin with 0xBB";
        return false;
    }
    if(ba[31] != CRC8(buf, len - 1))
    {
        qDebug() << "recv data CRC fail";
        return false;
    }
//    qDebug() << "IsReplyPacket ok";
    return true;
}

 bool message::IsEnterTestMode(QByteArray reply)
 {
     if(IsReplyPacket(reply) && reply.at(1) == ENTER_TEST_MODE)
     {
          qDebug() << "IsEnterTestMode ok";
         return true;
     }
     else
     {
         qDebug() << "IsEnterTestMode fail";
         return false;
     }
 }

 bool message::GetNordicSoftVerFromData(QByteArray reply, QString& s)
 {
     if(!IsReplyPacket(reply) && reply.at(1) == GET_NORDIC_SOFTVER)
     {
         qDebug() << "GetNordicSoftVerFromData fail";
         return false;
     }
     QByteArray ba= reply.mid(2, 6);
     qDebug() << "ba:" << ba.toHex();
     s = ba;
     qDebug() << "s:" << s;
     return true;
 }

 bool message::GetNordicSNFromData(QByteArray reply, QString& s)
 {
     if(!IsReplyPacket(reply) && reply.at(1) == GET_NORDIC_SN)
     {
         qDebug() << "GetNordicSNFromData fail";
         return false;
     }
     QByteArray ba= reply.mid(2, 17);
     qDebug() << "ba:" << ba.toHex();
     s = ba;
     qDebug() << "s:" << s;
     return true;
 }

